    ...
    return (new class($client) extends \CharlotteDunois\Livia\Commands\Command {
        function __construct(\CharlotteDunois\Livia\Client $client) {
            parent::__construct($client, array(
                ...
                'aliases' => array(),
                'group' => 'settings',
                'description' => 'Показывает или изменяет преффикс команд.',
        ...
        function run(\CharlotteDunois\Livia\Commands\Context $context, \ArrayObject $args, bool $fromPattern) {
            if(empty($args['prefix'])) {
                ...
                $msg = ($prefix !== null ? 'командный префикс `'.$prefix.'`.' : 'Префикс не установлен.').\PHP_EOL.'Пример использования команд: '.\CharlotteDunois\Livia\Commands\Command::anyUsage('command', $prefix, $this->client->user).'.';
